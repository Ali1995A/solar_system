...
for(const [index,[name,base]] of bodies.filter(([name])=>name!=='earth').entries()) {
    ...
    for(let y=0;y<size;y++)for(let x=0;x<size;x++) {
        ...
        for(let k=0;k<3;k++) img.data[dst+k]=pixels[src+k]*light;img.data[dst+3]=255;
    }
...
console.log('PASS: nine procedural texture generators, material maps, ring gap, transparent corona center. Earth uses a separate NASA asset.');
